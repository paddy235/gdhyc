...
<base href="<%=basePath%>">
<div class="tab-pane" ng-controller="erjshCtrl">
	<div class="block-content collapse in ">
		<div class="span12">
			<div class="table-toolbar">
				<label style="width:40px;height:30px;line-height:30px; float: left;" class="control-label span1">单位:</label>
				<select style="width: 120px; float: left;" id="meik" ng-model="search.diancID" 
					ng-change="load()"
				...
				<select style="width: 180px; float: left;" ng-model="search.zhuangt"
					ng-change="load()"
					>
					<!-- <option value=1 >一级审核</option>	 -->
					<option value=1 >二级审核通过</option>
				</select>
				...
				<div id="example_filter" class="dataTables_filter">
					<input type="search" class="" style="float:right;" placeholder="请输入化验时间或化验编码" aria-controls="example" ng-model="search.condition" 
						ng-change="load()" />
			...
			<table class="table table-striped table-bordered table-hover"
				...
				<tbody>
					<tr ng-repeat="row in list track by $index" class="option">
						...
						<td style="text-align: center; width: 80px;"><br> <br>
							<br>
							<button  id="shenh" ng-click="shenhtg($event.target,row.ID,search.zhuangt)" class="btn btn-success">
								<i class="icon-plus icon-white"></i> 审核
							</button> <br> <br> <br>
<!-- 							<button  class="btn btn-primary" id="adddata" ng-click="huit($event.target,row.ID,search.zhuangt)">
