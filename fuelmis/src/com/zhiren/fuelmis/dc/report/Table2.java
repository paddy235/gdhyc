...
public class Table2 {
	...
	public Table2(int iRows, int iCols) {
		iniTable(iRows, iCols);
		iniTableData(defalutCell);
		getPages();
	}

	public Table2(int iRows, int iCols, Cell c) {
		iniTable(iRows, iCols);
		iniTableData(c);
		getPages();
	}
	
	public Table2(int iRows, int iCols, int[] colWidth, Cell c) {
		iniTable(iRows, iCols);
		iniTableData(c);
		setWidth(colWidth);
		getPages();
	...
	public Table2(ResultSet rs, int iFixedRows, int iFooterRows, int intFixedCols) {
		try {
			int i = 0;
			int j = 0;
			int iRows = 0;
			...
			while (rs.next()) {
				...
				for (j = 1; j <= miCols; j++) {
					...
					if (j > intFixedCols) {
						cells[i][j].align = Table2.ALIGN_RIGHT;
					}
				}
			}
	...
	public Table2(String[][] arrString, int iFixedRows,int iFooterRows, int intFixedCols) {
		int i = 0;
		int j = 0;
		if (arrString == null) {
			return;
...
//		复制原有的数据
	...
	public void InsertFixedRows(Table2 tb) {
		if(tb == null) {
			return;
		}
		int iRows=0;
...
//		填写表头数据
		...
		for (i = 1; i <= arrString.length; i++) {
			for (j = 1; j <= arrString[0].length; j++) {
				if (j <= miCols) {
					...
					cells[i][j].align = Table2.ALIGN_CENTER;
	...
	public void setFixedRowsAlign(int iAlign) {
		if (miRows>0){
			for (int i=1;i<=miFixedRows;i++){
				this.setRowCells(i, Table2.PER_ALIGN , iAlign);
			}
		}
	}
		...
		for (i = 1; i <= arrString.length; i++) {
			...
			for (j = 1; j <= arrString[0].length; j++) {
				if (j <= miCols) {
					...
					cells[i + iStartRow - 1][j + iStartCol - 1].align = Table2.ALIGN_CENTER;
	...
	public boolean fillRowData(String[] arrString, int iRow) {
		...
		for (j = 1; j <= arrString.length; j++) {
			...
			if (j <= miCols) {
				...
				cells[iRow][j].align = Table2.ALIGN_CENTER;
	...
	public void setColAlign(int iCol, int intAlign) {
		setColCells(iCol, Table2.PER_ALIGN, intAlign);
	}

	// 设置列垂直对齐
	public void setColVAlign(int iCol, int intAlign) {
		setColCells(iCol, Table2.PER_VALIGN, intAlign);
	}
...
//		 垂直对齐
		...
		if (c.value == null || c.value.trim().equals("")) {
			...
			sb.append("&nbsp;");
		}else if (c.value.equals(Table2.PAGENUMBER_NORMAL)) {
			sb.append("(第" + miCurrentPage+ "/" + miPages + "页)");
		}else if (c.value.equals(Table2.PAGENUMBER_CHINA)) {
...
//		}
	 ...
	 * 扩展
	 */
	public Table2(String[][] arrString, int iFixedRows,int iFooterRows, int intFixedCols,int iMiCols) {
		int i = 0;
		int j = 0;
		if (arrString == null) {
			return;
...
//		 垂直对齐
		...
		StringBuffer sbStyle =getCellStyle(i,j); 
		if (sbStyle.length()>0){
//			sb.append(" style='").append(sbStyle).append("width:"+cellWidth+"%;'");
			sb.append(" style='").append(sbStyle).append("'");
		}
