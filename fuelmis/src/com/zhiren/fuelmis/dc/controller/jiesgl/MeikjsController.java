...
public class MeikjsController {
    ...
    public @ResponseBody List<Map<String,Object>> getjsdList(@RequestParam String condition, HttpServletRequest request, HttpServletResponse response) {
        ...
        Object meikxxb_id=map.get("MEIKID");
        Object pinzb_id=map.get("PINZB_ID");
        ...
        "           c.pinz,\n" +
        "           count(*) AS ches,\n" +
        "           substr(zhongcsj,0,10) AS DAOHRQ,\n" +
        "           SUM(C.SANFSL) AS SANFSL,\n" +
        "           SUM(C.MAOZ) MAOZ,\n" +
        "           SUM(C.PIZ) PIZ,\n" +
        "           SUM(C.ZONGKD) ZONGKD,\n" +
        "           SUM(C.MAOZ - C.PIZ - C.ZONGKD) AS JINGZ       \n" +
        "           FROM rl_ys_chepb c INNER JOIN rl_ys_chepb_qc q ON c.id=q.chepb_id\n" +
        "           GROUP BY samcode,GONGYSB_ID,GONGYSMC,MEIKXXB_ID,MEIKMC,PINZB_ID,pinz,substr(zhongcsj,0,10)) s\n" +
        "           INNER JOIN \n" +
        "           -------------------------\n" +
        "           (SELECT DISTINCT g1.yuanbm samcode,g2.mubbm huaybm FROM \n" +
        "           (SELECT * FROM  gx_chep_caizhbmb WHERE zhuanhlb_id=1) g1\n" +
        "           INNER JOIN \n" +
        "           (SELECT * FROM  gx_chep_caizhbmb WHERE zhuanhlb_id=2) g2\n" +
        "           ON g1.mubbm=g2.yuanbm) b ON s.samcode=b.samcode INNER JOIN \n" +
        "           ---------------------------\n" +
        "            ( SELECT \n" +
        "             huaybm,AAR,\n" +
        "                  AD,\n" +
        ...
        "                  QGRD,\n" +
        "                  STAR,\n" +
        "                  decode(zhuangt,0,'未化验',1,'一审通过',2,'二审通过') AS huayzt,\n" +
        "                  QNET_AR\n" +
        "                  FROM rl_hy_huaydb ) h ON b.huaybm=h.huaybm INNER JOIN\n" +
        "                  -----\n" +
        "                  (SELECT  g.samcode,g.RUKDBH, SUM(k.JINE) RK_JINE,k.RL_JS_YUEJSDB_ID  FROM \n" +
        "                  (SELECT DISTINCT samcode,g.rukdbh FROM rl_ys_chepb c INNER JOIN gx_churkdb_yunsdj g ON c.id=g.yuandj_id) g\n" +
        "                  INNER JOIN  rl_kc_churkdb  k ON g.rukdbh=k.rukdbh\n" +
        "                  GROUP BY  g.RUKDBH,g.samcode,k.RL_JS_YUEJSDB_ID) r ON b.samcode=r.samcode\n" +
        "                  \n" +
        "                  WHERE RL_JS_YUEJSDB_ID is null and s.DAOHRQ   BETWEEN '"+sdate+"' AND '"+edate+"'\n" +
        (gongysb_id.toString().equals("-1")?"":"and GONGYSB_ID="+gongysb_id)+"\n"+
        (meikxxb_id.toString().equals("-1")?"":"and MEIKXXB_ID="+meikxxb_id)+"\n"+
        (pinzb_id.toString().equals("-1")?"":"and PINZB_ID="+pinzb_id)+"\n"+
        "order by s.DAOHRQ,GONGYSMC";
    ...
    public
    @ResponseBody
    Map<String, Object> getJiesdlist(String jiesbh, HttpServletRequest request, HttpServletResponse response) {
        response.setContentType("text/html;charset=UTF-8");
        try {
    ...
    public
    @ResponseBody
    Map<String, Object> getJiesd(@RequestParam(defaultValue = "") String id, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
    ...
    public
    @ResponseBody
    Map<String, Object> jies(String hetj, String rukdbhs, HttpServletRequest request, HttpServletResponse response) throws Exception {
        response.setContentType("text/html;charset=UTF-8");
        try {
            ...
            for (Map<String, Object> scheme : schemeList) {
                ...
                zengkmx.put("ZHEJBZ", Math.round(priceBean.getPrice(), 2));
            ...
            jiesdMap.put("HETZBZDJ", Math.round(jiesjg, 2));
            ...
            jiesdMap.put("BUHSDJ", CustomMaths.round(jiesjg / 1.17, 2));
            ...
            jiesdMap.put("MEIKJE", CustomMaths.round(jiesje, 2));
            jiesdMap.put("JIAKHJ", CustomMaths.round(jiesje, 2));
            jiesdMap.put("SHUIK", Math.round(jiesje - jiesje / 1.17, 2));
    ...
    public
    @ResponseBody
    List<Map<String, Object>> getGongysList(@RequestParam(defaultValue = "") String condition,
                                            HttpServletRequest request, HttpServletResponse response) {
        response.setContentType("text/html;charset=UTF-8");
        Map<String, Object> map = JSON.parseObject(condition);
        try {
            String sql = " select -1 id,'全部' mingc from dual UNION  all select  distinct r.gongysb_id id,r.gongysmc mingc from (\n" +
                    "select distinct c.gongysb_id,c.gongysmc,g.rukdbh from rl_ys_chepb c \n" +
                    "inner join gx_churkdb_yunsdj g on c.id=g.yuandj_id \n" +
                    "inner join rl_ys_chepb_qc q on c.id=q.chepb_id\n" +
                    "where substr(q.zhongcsj,0,10) between '" + map.get("sDate") + "' and '" + map.get("eDate") + "'\n" +
                    "and g.rukdbh is not null) r inner join rl_kc_churkdb k on r.rukdbh=k.rukdbh\n" +
                    "and k.rl_js_yuejsdb_id is null";
            List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
            return list;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void getMeikxxList(@RequestParam(defaultValue = "") String condition,
        ...
        Map<String, Object> map = JSON.parseObject(condition);
        PrintWriter writer;
        String sql = "  select  distinct r.MEIKXXB_ID id,r.meikmc mingc from (\n" +
                "select distinct c.meikxxb_id,c.meikmc,g.rukdbh from rl_ys_chepb c \n" +
                "inner join gx_churkdb_yunsdj g on c.id=g.yuandj_id \n" +
                "inner join rl_ys_chepb_qc q on c.id=q.chepb_id\n" +
                "where substr(q.zhongcsj,0,10) between '" + map.get("sDate") + "' and '" + map.get("eDate") + "'\n" +
                "and g.rukdbh is not null) r inner join rl_kc_churkdb k on r.rukdbh=k.rukdbh\n" +
