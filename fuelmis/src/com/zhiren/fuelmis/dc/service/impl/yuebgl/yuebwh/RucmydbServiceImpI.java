...
import com.zhiren.fuelmis.dc.dao.yuebgl.yuebwh.RucmydbDao;
...
import com.zhiren.fuelmis.dc.service.yuebgl.yuebwh.IRucmydbService;
 ...
 */
@Service
public class RucmydbServiceImpI implements IRucmydbService {
	@Autowired
	JdbcTemplate jdbcTemplate;
	@Autowired
	private RucmydbDao guodbbDao;
...
"   AND MEIKDQB.SHENGFB_ID = SHENGFB.ID";
	...
	public JSONArray getMeishcbb(Map<String, Object> map) {
		...
		String sql="SELECT qingcsj,sum(jingz) jingz,\n" +
				...
				"round(decode(sum(jiesjg), 0, 0, sum(jiesjg * jingz) / sum(jingz)), 2) jiesjg,\n" +
				...
				"round(sum(zongj),2) zongj,0 suopje,0 rulbd,0 changlbdc,0 rezc,0 kucrz,0 yuemkc\n" +
				" FROM \n" +
				" (SELECT j.qingcsj,(c.maoz-c.piz) jingz, h.mt,h.mad,h.ad,h.aar,h.std,h.vdaf,h.fcd,h.qnet_ar,j.jiesjg,(jiesjg) daocmj,\n" +
				"  (j.jiesjg*29.271/h.qnet_ar) daocbmj,(j.jiesjg/1.17) buhsmj,((j.jiesjg/1.17)*29.271/h.qnet_ar) buhsbmdj,(j.jiesjg*c.jingz) zongj,\n" +
				"  0 suopje,0 rulbd,0 changlbdc,0 rezc,0 kucrz,0 yuemkc\n" +
				" FROM rl_ys_chepb c\n" +
				...
				"  ON j.id=g.jiesdb_id where substr(j.qingcsj,0,7)='"+map.get("riq")+"') group by qingcsj";

		List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);

		String[][] arrData = new String[list.size()][22];
		...
		ArrHeader[0] = new String[] { "时间", "发电量", "入厂数量", "全水", "干基灰","全硫", "挥发分",  "热值", "含税单价", "含税标单", "不含税单价", "不含税标单", "总价","索赔金额","入炉标单","厂炉标单差", "热值差","库存热值差","月末库存","红二电入厂含税标单","对标差","区域排名"};
		ArrHeader[1] = new String[] { "时间", "千千瓦时", "（吨）", "Mt", "Ad", "st.d", "Vdaf", "Qver", "元/吨","元/吨","元/吨", "元/吨", "元/吨", "元", "元/吨","元/吨","MJ/kg", "MJ/kg","吨","元/吨","元/吨","区域排名"};



		int[] ArrWidth = new int[] {70,50,50,50,50,50,50,50,50,50,50, 50,50,50,50, 50, 50, 50,50, 50, 50, 50, 50,50 };
		Table titleTable = new Table(4,22);
		...
		rt.setTitle("国电新疆红雁池发电有限公司、入厂煤月度综合指标统计表", 2);
		rt.setBody(new Table(arrData, 3, 0, 0, 22));
