...
import com.zhiren.fuelmis.dc.dao.rib.RanmzhrbbDao ;
 ...
 */
@Service
public class RanmzhrbbService implements IRanmzhrbbService {

	@Autowired
	private RanmzhrbbDao RanmzhrbbDao;
...
//		String  dianc_mingc = niandjhtjDao.getDiancMingc(map.get("diancid").toString());
		...
		String[] column = {"MEIKMC","CHES_HJ","CHES_LJ",
				"KFSL_HJ","KFSL_LJ","KFSL_JH","KFSL_WCL",
				"GH_HJ","GH_LJ",
				"YUNS_HJ","YUNS_LJ",
				"KUID_HJ","KUID_LJ",
				"SUNH_HJ","SUNH_LJ",
...
//		rt.body.mergeFixedCols();
	...
	@SuppressWarnings("unchecked")
	@Override
	public String getShouhcDetail(Map<String,Object> map){
		String sr="LJ";

        Map<String, Object> m = jdbcTemplate.queryForMap("select max(mingc) mingc from diancxxb WHERE id="+map.get("diancid"));

		
		List<Map<String, Object>> list = RanmzhrbbDao.getShouhcDetail(map);
		
		String[] column = {"ZRKC","RUCSL", "RISML_1","RISML_2","RISML_HJ","RISML_1_LJ","RISML_2_LJ","RISML_LJ","YUNS","KUID","CHANGH","PANYK"};
		String[][] arrData = DrawTableUtil.Creatarr(list,column);
		Report rt = new Report();
		String ArrHeader[][]=new String[3][column.length];
//		if(getBBLXValue().getId()==1){

        ArrHeader[0]=new String[] {"昨日库存","当日入厂总量","上煤量","上煤量","上煤量","上煤量","上煤量","上煤量","当日实际运损","当日实际运损","","","当日库存"};
        ArrHeader[1]=new String[] {"昨日库存","当日入厂总量","日上煤量","日上煤量","日上煤量","累计上煤量","累计上煤量","累计上煤量","当日实际运损","当日实际运损","","","当日库存"};
        ArrHeader[2]=new String[] {"昨日库存","当日入厂总量","#1机","#2机","合计","#1机","#2机","合计","运损","运损","场耗","盘盈盘亏","当日库存"};
...
*/
		int ArrWidth[]=new int[] {100,100,100,100,100,100,100,100,100,100,100,100,100};
		//rs.beforefirst();

		rt.setBody(new Table(arrData, 3, 0, 0 , 13));
		rt.body.setHeaderData(ArrHeader);
