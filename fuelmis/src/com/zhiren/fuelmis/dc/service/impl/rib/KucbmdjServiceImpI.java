...
import com.zhiren.fuelmis.dc.dao.rib.KucbmdjDao;
 ...
 */
@Service
public class KucbmdjServiceImpI implements IKucbmdjService {

	@Autowired
	private KucbmdjDao RanmzhrbbDao;
...
//		rt.body.mergeFixedCols();
	...
	public JSONArray getShouhcDetail(Map<String,Object> map){
		...
		String[] column = {"RIQ","RUCBD","RULBD","KUCBD","KUCJRLSJ","RULJRCSJ"};
		String[][] arrData = DrawTableUtil.Creatarr(list,column);
		Report rt = new Report();
		String ArrHeader[][]=new String[1][column.length];

        ArrHeader[0]=new String[] {"年月","入厂标单","入炉标单","库存标单","库存较入炉升降","入炉较入厂升降"};
		...
		rt.setTitle("国电电力发展股份有限公司入厂、入炉、库存标煤单价变化数据表", 2);
