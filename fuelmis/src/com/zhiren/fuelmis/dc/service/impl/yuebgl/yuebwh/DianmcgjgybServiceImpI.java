...
import com.zhiren.fuelmis.dc.dao.yuebgl.yuebwh.DianmcgjgybDao;
 ...
 */
@Service
public class DianmcgjgybServiceImpI implements IDianmcgjgybService {
	@Autowired
	JdbcTemplate jdbcTemplate;
	@Autowired
	private DianmcgjgybDao guodbbDao;
...
"   AND MEIKDQB.SHENGFB_ID = SHENGFB.ID";
	...
	public JSONArray getMeishcbb(Map<String, Object> map) {
		...
		String sql=
			"select * from \n" +
					"(select '合计' qingcsj, sum(jingz) jingz,\n" +
					...
					"round(decode(sum(jiesjg), 0, 0, sum(jiesjg * jingz) / sum(jingz)), 2) jiesjg,0 yunf,0 zaf,\n" +
					"\n" +
					...
					"  ON j.id=g.jiesdb_id\n" +
					"  inner join yuebsjpzb y on substr(q.qingcsj,0,10) between  y.yuebksrq and y.yuebjzrq\n" +
					"   WHERE y.yuebjq='"+map.get("riq")+"' \n" +
					"  ) t)\n" +
					"   ,\n" +
					"   (select\n" +
					...
					"  where substr(rulrq,0,7)='"+map.get("riq")+"'\n" +
					") \n" +
					"union all\n" +
					"--------------------------------------------------------------\n" +
					"select t.qingcsj,sum(jingz) jingz,\n" +
					"round_new(decode(sum(mt), 0, 0,sum(mt * jingz) / sum(jingz)),2) mt,\n" +
					"round_new(decode(sum(mad), 0, 0,sum(mad * jingz) / sum(jingz)),2) mad,\n" +
					"round_new(decode(sum(ad), 0, 0,sum(ad * jingz) / sum(jingz)),2) ad,\n" +
					"round_new(decode(sum(aar), 0, 0,sum(aar * jingz) / sum(jingz)),2) aar,\n" +
					...
					"  from(\n" +
					"  select j.qingcsj,maoz-piz jingz, h.mt,h.mad,h.ad,h.aar,h.std,h.vdaf,h.fcd,h.qnet_ar,j.jiesjg,(jiesjg) daocmj,\n" +
					"  (j.jiesjg*29.271/h.qnet_ar) daocbmj,(j.jiesjg/1.17) buhsmj,((j.jiesjg/1.17)*29.271/h.qnet_ar) buhsbmdj \n" +
					" from rl_ys_chepb c\n" +
					"  inner join rl_ys_chepb_qc q\n" +
					"  on c.id = q.chepb_id\n" +
		...
		String[][] arrData = new String[list.size()][21];
		...
		ArrHeader[0] = new String[] { "日期", "数量", "全水", "干基水", "干基灰", "应基灰", "全硫", "挥发分", "固定碳", "热值", "煤价", "运费", "杂费", "到厂煤价", "到厂标煤价","不含税煤价","不含税标煤单价","入炉煤量", "入炉热值","热值差","入炉标煤单价"};
		ArrHeader[1] = new String[] { "日期", "吨", "Mt", "Mad", "Ad", "Aar", "st.d", "Vdaf", "FCd", "Qver", "元/吨", "元/吨", "元/吨", "元/吨", "元/吨","元/吨","元/吨", "吨","兆焦/千克","兆焦/千克","元/吨"};



		int[] ArrWidth = new int[] { 80, 55, 55, 55, 55, 55, 55, 55, 55, 55,55, 50, 50, 50, 50, 50, 50, 50,50, 50, 50, 50, 50 };
		Table titleTable = new Table(4,21);
		...
		rt.setTitle("国电新疆红雁池发电有限公司、电煤采购价格月报", 2);
		rt.setBody(new Table(arrData, 3, 0, 0, 21));
