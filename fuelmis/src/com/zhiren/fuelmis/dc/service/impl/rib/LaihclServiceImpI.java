...
import com.zhiren.fuelmis.dc.dao.rib.LaihclDao;
 ...
 */
@Service
public class LaihclServiceImpI implements ILaihclService {

	@Autowired
	private LaihclDao RanmzhrbbDao;
...
//		rt.body.mergeFixedCols();
	...
	public JSONArray getShouhcDetail(Map<String,Object> map){
		String sql="select d.riq, biaomdj ,c.kuc,c.shouml, (c.fady+c.gongry) haoml from \n" +
		...
		String[] column = {"RIQ","KUC","SHOUML","HAOML","BIAOMDJ"};
		String[][] arrData = DrawTableUtil.Creatarr(list,column);
		Report rt = new Report();
		String ArrHeader[][]=new String[1][column.length];

        ArrHeader[0]=new String[] {"年月","月末库存","进煤量","耗煤量","入厂标单"};

		int ArrWidth[]=new int[] {100,100,100,100,100};
		...
		rt.setBody(new Table(arrData, 1, 0, 0 , 5));
		rt.body.setHeaderData(ArrHeader);
		...
		rt.setTitle("国电电力发展股份有限公司来耗存量与入厂标煤单价变化数据表", 2);
