...
public class JiesbbServiceImpl implements IJiesbbService {
	...
	public JSONArray getJiesd(Map<String, Object> map) {
		...
		if(maps.get("JIESLX").equals("杂费结算" ) ){
			...
			buhsmj = danjhj / 1.17;
			shul = Double.parseDouble(maps.get("JINGZ").toString());
			...
			bukk = Double.parseDouble(maps.get("BUKK").toString()); //补扣
			mk_yf_hj = Double.parseDouble(maps.get("JIAKHJ").toString()); // 煤款应付合计
		    mk_yf_bhsje =Math.round(Double.parseDouble(maps.get("MEIKHJ").toString())/1.17,2); //煤款应付不含税
		    mk_yf_sk = Double.parseDouble(maps.get("SHUIK").toString()); //煤款应付税款
		    mk_yf_hsdj = Double.parseDouble(maps.get("JIESJG").toString()); //煤款含税单价
		}
		...
		if(map3!=null){
			...
			ArrBody[6] = new String[] { rez, rez_dk, huif , huiff, mt , liuf, 
					df_2.format(bukk),"", df_2.format(meikhj), "", df_2.format(meikhj-mk_yf_bhsje),
					"", df_2.format(mk_yf_bhsje), "", df_2.format(mk_yf_hsdj), "", "", "", "" };
		}else{ 
		...
		ArrBody[13] = new String[] { "" + "" + "", "", "", "", "" + "" + "", "", "", "", "", "", "" + "" + "", "", "",
				"", "", "", "" + "" + "", "", "" + "" + "" };
		String MEIKDJDX = NumberToCN.number2CNMontrayUnit(BigDecimal.valueOf(Double.parseDouble(maps.get("MEIKHJ").toString())));
		ArrBody[14] = new String[] { "合计", MEIKDJDX, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "" };
		ArrBody[15] = new String[] { "注:"+maps.get("BEIZ").toString() , "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "" };
		...
		rt.createDefautlFooter(ArrWidth);
		rt.setDefautlFooter(1, 3, "审核人:", Table.ALIGN_LEFT);
	...
	public JSONArray getYansmx(Map<String, Object> map) {
		...
		Report rt = new Report(); // 报表定义lihaiwodege
	...
	public JSONArray getDanpcmx(Map<String, Object> map) {
		...
		List<Map<String, Object>> lst = jdbcTemplate.queryForList(
				"SELECT cp.MEIKMC,\n" +
						"       max(substr(qc.qingcsj, 0, 10)) laimrq,\n" + 
						"       cp.faz as CHEZMC,\n" + 
						"       COUNT(1) CHES,\n" + 
						"       SUM(Cp.PIAOZ) PIAOZ,\n" +
