 ...
 */
@Service
public class CaigrkServiceImpl implements ICaigrkService {

	@Autowired
	private CaigrkDao caigrkDao;
	@Autowired
	private JsDao jsDao;
	@Autowired
	...
	public JSONObject getChurkdList(Map<String, Object> map) {
		List<Map<String, Object>> list = caigrkDao.getChurkdList(map);
	...
	public JSONObject getYansmx(Map<String, Object> map) {
		List<Map<String, Object>> list = caigrkDao.getYansmx(map);
		Map<String, Object> jsonMap = new HashMap<String, Object>();
    ...
    public JSONArray ranlcgrk(String ids, Diancxx diancxx, int yewlx) {
        String idList = jdbcTemplate.queryForObject("select wm_concat(ID) from RL_YS_CHEPB where SAMCODE = '" + ids +"'", String.class);
        String rukdbh = createRukdbh();
        if (yewlx == 1) {
            if (idList.length() > 0) {
                String[] arrId = idList.split(",");
                String hangh_str = jdbcTemplate.queryForObject(
                        "select max(HANGH) from RL_KC_CHURKDB",
                        String.class);
                int hangh = 0;
                if (hangh_str != null) {
                ...
                for (int i = 0; i < arrId.length; i++) {
                    ...
                    hangh = hangh + 1;
                    // 获取验收明细
                    Map<String, Object> chepmx = jdbcTemplate.queryForMap("select  c.id, c.xuh, c.cheph, c.piaojh, c.maoz, c.piz, c.piaoz,\n"
                            ...
                            + "c.kuid, c.meic, c.sanfsl, c.chec, P.MINGC PMINGC,bm.huaybm \n"
                            + " from RL_YS_CHEPB C,PINZB P,vm_caizhbm bm \n"
                    ...
                    String huaybm = chepmx.get("huaybm").toString();
                    
                    String yewrq = "";
                    if (yunsfs.equals("2")) {
                        yewrq = jdbcTemplate.queryForObject("select QINGCSJ from RL_YS_CHEPB_QC where CHEPB_ID = " + arrId[i], String.class);
                    }
                    // 获取库存位置
                    String kucwz = "0";
                    kucwz = jdbcTemplate.queryForObject(
                                    "select distinct fuid as ID from RL_KC_KUCWZB where KUCZZ_ID in (select ID from RL_KC_KUCZZB where SUOSDW = "
                                            + diancxx.getId() + ") and KUCWZMS = '" + chepmx.get("MEIC") + "'", String.class);
					...
					sb.append("CHURKLX,GONGYSB_ID,MEIKXXB_ID,PINZB_ID,MAOZ,PIZ,PIAOZ,HANSJE,jizrq ,HUAYBH,SAMECODE,shifdr) values( \n");
					...
					sb.append(  piaoz + "," + hansje + ",'" + jizrq + "','"+huaybm+"','"+samcode+"','0') ;\n");
			...
			if (idList.length() > 0) {
				...
				for (int i = 0; i < arrId.length; i++) {
					...
					// 插入RL_KC_CHURKDB
					String sql = "insert into RL_KC_CHURKDB(ID,RUKDBH,ZUZ,KUCZZ,KUCWZ,KUCWL,HUOZ,JINE,YANSSL,HANGH,ZHUANGT,YEWLX,YEWRQ,shifdr) values("
							+ Sequence.nextId() + ",'" + rukdbh + "'," + diancxx.getId() + ",0," + kucwz + "," + kucwl
							+ "," + diancxx.getId() + "," + jine + "," + yanssl + "," + hangh + ",-1," + yewlx + ",'"
							+ DateUtil.format(new Date(), DateFormatType.SIMPLE_TYPE) + "','0')";
		...
		if (yewlx == 1) {
			if (list != null) {
				for (int i = 0; i < list.size(); i++) {
					...
					result += jdbcTemplate.update("update RL_KC_CHURKDB set kucwz = " + map.get("kucwz") + ",kucwl = "
							+ map.get("kucwl") + ",kuczz = " + kuczz + ",huoz = " + huoz + ",yewlx = 1,shifdr='0' where rukdbh = '"
			...
			if (list != null) {
				for (int i = 0; i < list.size(); i++) {
					Map<String, Object> map = list.get(i);
					result += jdbcTemplate.update("update RL_KC_CHURKDB set kucwz = " + map.get("kucwz") + ",kucwl = "
							+ map.get("kucwl") + ",kuczz = " + kuczz + ",huoz = " + huoz + ",yewlx = 2,shifdr='0' where rukdbh = '"
							+ rukdbh + "' and hangh =" + map.get("hangh"));
				}
			}
		} else {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (map.get("hangh") == null || map.get("hangh").equals("")) {
						String sql = "insert into RL_KC_CHURKDB(ID,RUKDBH,ZUZ,KUCZZ,KUCWZ,KUCWL,HUOZ,JINE,TIAOZJE,YEWLX,"
								+ "RUKSL,HANGH,ZHUANGT,YEWRQ,shifdr) values (" + Sequence.nextId() + ",'" + rukdbh + "',"
						...
						String sql = "update RL_KC_CHURKDB set kucwz = " + map.get("kucwz") + ",kucwl = "
								...
								+ (map.get("shul") == null || "".equals(map.get("shul").toString()) ? "0"
										: map.get("shul"))
								+ "shifdr='0' where rukdbh = '" + rukdbh + "' and hangh =" + map.get("hangh");
						result += jdbcTemplate.update(sql);
					}
				}
			}
		...
		try {
			...
			jdbcTemplate.update(
					"update RL_KC_CHURKDB set jizrq = '" + DateUtil.format(new Date(), DateFormatType.SIMPLE_TYPE)
							...
							+ "',zhuangt = 1,shifdr='0' where rukdbh = '" + rukdbh + "'");
//			this.meikrk(map);
//			this.shicbhs(rukdbh);
			result = 1;
		} catch (Exception e) {
	...
	public JSONObject getRanlcgrk_WRK_MX2(Map<String, Object> map) {
		...
		try {
			if (!rukdbh.equals("")) {
				String sql = "select id,HANGH,KUCWZ,KUCWL,YUNDSL,YANSSL,RUKSL,JIESSL,JINE,RUKDBH,TIAOZJE,ZHUANGT from RL_KC_CHURKDB where RUKDBH ='"
						+ rukdbh + "'";
				List<Map<String, Object>> list = jdbcTemplate.queryForList(sql);
				JSONArray jsonArray = new JSONArray();
				for (int i = 0; i < list.size(); i++) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				Map<String, Object> mapChep = jdbcTemplate.queryForMap(
						"SELECT (C.MAOZ-C.PIZ-c.zongkd) SHUL,(Z.STAD*(100-Z.MT)/(100-Z.MAD)) STAR,(Z.QNET_AR*1000/4.1816) QNET_AR "
								...
								+ list.get(i).get("CHEPB_ID") + " AND Z.HUAYD_ID = " + list.get(i).get("HUAYD_ID"));
				// 调用其他方法，计算价格
				...
				// String.class);
				String sql = "insert into RL_KC_CHURKDB(ID,RUKDBH,ZUZ,KUCZZ,KUCWZ,KUCWL,HUOZ,JINE,YANSSL,HANGH,ZHUANGT,YEWLX,YEWRQ,JIZRQ,shifdr) values("
	...
	public JSONObject getCaigddList(Map<String, Object> map) {
		List<Map<String, Object>> list = caigrkDao.getCaigddList(map);
		Map<String, Object> jsonMap = new HashMap<String, Object>();
		Object[][] objs = new Object[list.size()][];
		for (int i = 0; i < list.size(); i++) {
			Map<String, Object> rtnMap = list.get(i);
		...
		try {
			...
			for (int i = 0; i < lst.size(); i++) {
				...
				jdbcTemplate.update("update RL_KC_CHURKDB set JINE = " + shul + "*" + danj + ",kucwl = " + kucwl
						+ "shifdr='0' where RUKDBH = '" + rukdbh + "' and HANGH = " + lst.get(i).get("HANGH"));
	...
	public JSONObject getChurkdList2(Map<String, Object> map) {
		List<Map<String, Object>> list = caigrkDao.getChurkdList2(map);
		Map<String, Object> jsonMap = new HashMap<String, Object>();
		List<Map<String, Object>> rtnList = new ArrayList<Map<String, Object>>();
		for (int i = 0; i < list.size(); i++) {
			Map<String, Object> rtnMap = list.get(i);
	...
	public JSONObject getYansmxMX(Map<String, Object> map) {
		List<Map<String, Object>> list = caigrkDao.getYansmxMX(map);
		Map<String, Object> jsonMap = new HashMap<String, Object>();
		Object[][] objs = new Object[list.size()][];
		for (int i = 0; i < list.size(); i++) {
			Map<String, Object> rtnMap = list.get(i);
