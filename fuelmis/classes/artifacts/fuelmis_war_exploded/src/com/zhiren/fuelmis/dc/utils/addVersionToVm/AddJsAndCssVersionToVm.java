package com.zhiren.fuelmis.dc.utils.addVersionToVm;

import com.zhiren.fuelmis.dc.utils.MD5FileUtil;
...
public class AddJsAndCssVersionToVm {
     ...
     * 默认html 文件路径
     */
    private static String path = "views/main/";
    /**
     * 默认js 文件路径
     */
    private static String jspath = "js/views/";
    /**
     * 初始化
     * @Version:
     */
    public static void execute(String realPath) {
        Date date = new Date();
        SimpleDateFormat df = new SimpleDateFormat("yyyyMMddhhmmss");
        String version = df.format(date);
        Map<String, String> map = getJsMap(realPath);
        addVersionToVm(realPath, version,map);
    }
    /**
     * 获取js 与css 版本map
     * @Version:
     ...
     * @return
     */
    public static Map<String, String> getJsMap(String realPath) {
...
//                        System.out.println(strFileName + "---------------" + md5 + "-----" + files[i].getName());
//                        System.out.println(files[i].getPath());
     ...
     * @param map 版本map
     */
    private static void addVersionToVm(String realPath,String version,Map<String,String> map){
        File dir = new File(realPath+path);
        File[] files = dir.listFiles();
        if (files == null){
            return;
        ...
        for (int i = 0; i < files.length; i++) {
            if (files[i].isDirectory()) {
                ...
                String strFileName = files[i].getAbsolutePath().toLowerCase();
                // 如果是符合条件的文件，则添加版本信息
                if (strFileName.endsWith(".vm")|| strFileName.endsWith(".html")|| strFileName.endsWith(".jsp")) {
    ...
    private static String getModLine(String line, String version, Map<String, String> map) {
        ...
        if (line.startsWith("<script") && line.endsWith("</script>")) {
            ...
            // 默认加时间为版本号
            String modLine = line.substring(0, pos) + ".js?version=" + version+ "\"></script>";
            ...
            for(int i=0;i<nameStr.length;i++){
                ...
                if(jsname.indexOf(".js")!=-1){
                    ...
                    if(md5!=null){
                        modLine = line.substring(0, pos) + ".js?version=" + md5+ "\"></script>";
                    }
