 ...
 */
@Component("FourCalculatorService")
public class FourCalculatorService implements Icalculator {
...
//		long maxrezhi=5500;
		...
		if (rangeList!=null&&rangeList.size()>0) {
			...
			//（验收指标-合同基准）/增扣幅度*增扣价（元/千大卡）/1000*结算热值+ 基准增扣价
			double hej=jiangkjs*shijzkj/1000*jiesvalue;
			if(hej>0){
				shijjg=(zhibvalue-jizrz)/jiangkjs*shijzkj/1000*jiesvalue+jizkj;
			}else{
				shijjg=jizkj;
			}
			BigDecimal bigjine = new BigDecimal(shijjg);
			shijjg = bigjine.setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
            String formula="("+zhibvalue+"-"+jizrz+")/"+jiangkjs+"*"+shijzkj+"/"+"1000"+"*"+jiesvalue+"+"+jizkj;
