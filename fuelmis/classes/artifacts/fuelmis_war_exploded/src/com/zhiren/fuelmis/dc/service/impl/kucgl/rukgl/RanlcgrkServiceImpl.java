 ...
 */
@Service
public class RanlcgrkServiceImpl implements IRanlcgrkService {
					...
					try {
						...
						try {
							...
							String caigddSql = "select G.MINGC GONGYS_ID,C.KAISSJ,C.JIESSJ,Y.MINGC YUNSFS_ID,L.WUZMC HUOW_ID"
									...
									+ " where C.ID = S.CAIGDDB_ID \n "
									+ "   and C.GONGYS_ID = G.ID and S.YUNSFS_ID = Y.ID\n"
									+ "   and (select mingc from pinzb where id = s.pinz_id) = L.Wuzmc \n"
									+ "   and C.ID = " + caigdd_id;
